 ...
 */
function msr_project_meta_shortcode($atts) {
 ...
 * Post Voting Buttons
 */
function msr_post_voting_buttons($atts) {
    ...
    $html = '<div class="msr-voting-buttons">';
        $html .= sprintf(
            '<button class="msr-like" data-post-id="%s">%s</button>',
            esc_attr( $post_id ),
            esc_html( $attrs['like'] )
        );
        $html .= sprintf(
            '<button class="msr-dislike" data-post-id="%s">%s</button>',
            esc_attr( $post_id ),
...
add_shortcode( 'VOTING_BUTTONS', 'msr_post_voting_buttons' );
