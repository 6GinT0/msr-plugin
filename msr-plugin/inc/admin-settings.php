...
function msr_settings_init() {
    ...
    register_setting('msrplugin_sub', 'msr_setting_field_checbox');
...
function msr_checkbox_settings_field_callback() {
	// get the value of the setting we've registered with register_setting()
	$setting = get_option('msr_setting_field_checbox');
    //print_r($setting);
	// output the field
	?>
	<input type="checkbox" name="msr_setting_field_checbox" <?php echo ($setting == 'on' ? 'checked' : '') ?>>
    <?php
}