<?php
function msr_post_voting_callback() {

    global $wpdb;
    $table_votes = $wpdb->prefix . 'post_votes';

    // CSRF protection
    check_ajax_referer( 'msr_vote_nonce', 'nonce' );

    $post_id = isset($_POST['pid']) ? intval($_POST['pid']) : 0;
    $user_id = get_current_user_id();
    $vote_type = isset($_POST['type']) ? sanitize_text_field($_POST['type']) : '';

    // Check user login status and capability
    if( !is_user_logged_in() || !current_user_can( 'read' )) {
        wp_send_json_error( [
            'message'    => esc_html__( 'You must be logged-in to vote', 'msr-plugin'),
        ] );
    }

    // Check if post exisst
    if( !get_post_status($post_id)) {
        wp_send_json_error( [
            'message'    => esc_html__('Invalid Post ID', 'msr-plugin'),
        ] );
    }

    // Check vote string
    if( ! in_array($vote_type, ['like', 'dislike'], true)) {
        wp_send_json_error( [
            'message'    => esc_html__('Invalid vote type', 'msr-plugin'),
        ] );
    }
    
    if(!empty($post_id) && !empty($user_id)) {

        // check for existing vote
        $exists = $wpdb->get_var( $wpdb->prepare(
            "SELECT COUNT(*) FROM {$table_votes} WHERE post_id = %d AND user_id = %d AND vote_type = %s",
            $post_id,
            $user_id,
            $vote_type
        ) );

        if ( $exists ) {
            wp_send_json_error( [ 'message' => 'You have already voted' ] );
        } else {
            $inserted = $wpdb->insert(
                $table_votes,
                [
                    'post_id'    => $post_id,
                    'user_id'    => $user_id,
                    'vote_type'=> $vote_type,
                ],
                [
                    '%d',
                    '%d',
                    '%s',
                ]
            );

            if ( false !== $inserted ) {
                wp_send_json_success( [ 'message' => 'Your vote has been recorded' ] );
            } else {
                wp_send_json_error( [
                    'message'    => 'Database error: ' . $wpdb->last_error,
                ] );
            }
        }

    }
}
add_action('wp_ajax_msr_post_voting', 'msr_post_voting_callback');
add_action('wp_ajax_nopriv_msr_post_voting', 'msr_post_voting_callback');